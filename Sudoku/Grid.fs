...
type Cell<'a> = { Row: int; Column: int; Region: int; Content: 'a }
...
let Update grid updates =
    ...
    updates |> Seq.iter (fun cell -> newCells.[cell.Column, cell.Row] <- {Row = cell.Row ; Column = cell.Column; Region = cell.Region; Content = Some cell.Content })
    { Cells = newCells; Size = grid.Size; MaxValue = grid.MaxValue }

let EmptyCells grid =
    grid.Cells |> Seq.cast<(Cell<int option>)> |> Seq.filter (fun cell -> cell.Content = None) |> Seq.length
